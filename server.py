...
# 时间片弹幕处理相关模型
class TimeSlotDanmuRequest(BaseModel):
    danmu_list: List[Dict[str, Any]]  # 时间片内的弹幕列表
    time_slot_start: str  # 时间片开始时间
    time_slot_duration: int  # 时间片长度（秒）
    model: str = "super-model"  # 使用的模型

class TimeSlotDanmuResponse(BaseModel):
    central_theme: str  # 中心思想
    selected_danmu: Dict[str, Any]  # 选中的最佳弹幕
    all_themes: List[str]  # 所有识别出的主题
    
@app.post("/api/live/analyze-time-slot", response_model=ApiResponse)
async def analyze_time_slot_danmu(request: TimeSlotDanmuRequest):
    """
    分析时间片内的弹幕，提取中心思想并选择最佳弹幕进行回复
    """
    try:
        # 构建分析提示词
        danmu_contents = [f"{i+1}. {danmu['content']}" for i, danmu in enumerate(request.danmu_list)]
        danmu_text = "\n".join(danmu_contents)
        
        analyze_prompt = f"""
请分析以下时间片内的弹幕内容（时间片长度：{request.time_slot_duration}秒，共{len(request.danmu_list)}条弹幕）：
...
1. 提取这些弹幕的中心思想和主要话题
2. 识别出最能代表这个中心思想的弹幕
3. 返回格式必须是严格的JSON格式，包含以下字段：
{{
    "central_theme": "中心思想描述",
    "selected_danmu_index": 最佳弹幕的序号（从1开始）,
    "all_themes": ["主题1", "主题2", "主题3"],
    "reason": "选择这条弹幕的原因"
}}

注意：
- 选择的弹幕应该是最能代表整体讨论焦点的
- 如果弹幕内容杂乱，选择最有意义或最能引发互动的那条
- 确保返回的是合法的JSON格式
"""
        ...
        response = await client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "你是一个专业的弹幕内容分析师，擅长从大量弹幕中提取核心思想和选择最佳互动内容。"},
        ...
        if json_match:
            ...
            # 获取选中的弹幕
            selected_index = result_data.get('selected_danmu_index', 1) - 1
            if 0 <= selected_index < len(request.danmu_list):
                selected_danmu = request.danmu_list[selected_index]
            else:
                selected_danmu = request.danmu_list[0] if request.danmu_list else {}
            
            return ApiResponse(
                success=True,
                data={
                    "central_theme": result_data.get('central_theme', ''),
                    "selected_danmu": selected_danmu,
                    "all_themes": result_data.get('all_themes', []),
