...
let vue_methods = {
  ...
  async processDanmuQueue() {
    try {
      ...
      if (oldestDanmu && oldestDanmu.content) {
        ...
        // 将弹幕内容赋值到用户输入
        this.userInput = "任务背景：你作为智能主播，需要分析处理下方提供的弹幕。\n" +
            "核心任务：从这批弹幕中，识别出重复出现或内容相似的讨论点，归纳出最主要的话题。注意，有可能只有一条弹幕消息，注意你的措词。\n" +
            "回答要求：\n" +
            "- 如果是普通弹幕：\n" +
            "   - 仅针对识别出的最主要话题，给出一个简短、自然的口语化回应。\n" +
            "   - 此回应应能覆盖该话题下多个相似弹幕的意图。\n" +
            "   - 无需问候，无需解释，直接给出回应内容。" +
            "- 如果是礼物/SuperChat/舰长：\n" +
            "   - 你需要问候，并且读出用户的名称，SuperChat需要复述发送的内容" +
            "   - 你需要表示感谢，并对内容做出简短回应" + "内容如下：【" + oldestDanmu.content + "】";
  ...
  handleDanmuMessage(data) {
    ...
    if (data.type === 'message') {
      ...
      console.log('收到新弹幕:', danmuItem.content, '当前队列长度:', this.length);
